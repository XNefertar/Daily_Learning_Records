...
![image-20241022231135116](./PNG/png1.png)
...
**四次挥手**与**三次握手**不同，其没有严格的次序要求，即客户端或服务端均可以发起断开连接请求（这里以客户端先发起请求为例）：
...
最后在再解释一下为什么是2 * MSL——因为MSL表示TLC报文段在网络中的最大生存时间，所以使用2 * MSL将保证任何报文传送在该时间段内必然会完成，且那些未及时传送到对端的数据会被_中转路由器 _完全丢弃，从而避免上述问题；
